...
pushd $FW_TARGETDIR >/dev/null
    ...
    if [ $RTOS != "host" ]; then
        ...
        rosdep install --os=ubuntu:resolute -y --from-paths $DEV_WS_DIR -i $DEV_WS_DIR --rosdistro $ROS_DISTRO --skip-keys="$SKIP"
...
rosdep install --os=ubuntu:resolute -y --from-paths $PREFIX/config/$RTOS/$TARGET_FOLDER -i $PREFIX/config/$RTOS/$TARGET_FOLDER --rosdistro $ROS_DISTRO --skip-keys="$SKIP"
